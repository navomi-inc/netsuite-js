...
var Configuration = module.exports = function Configuration(credentials, options, netsuiteType) {
	console.log("[NESTUITE-JS][CONFIGURATION] credentials: ", credentials);
	console.log("[NESTUITE-JS][CONFIGURATION] options: ", options);
	console.log("[NESTUITE-JS][CONFIGURATION] netsuiteType: ", netsuiteType);
...
Configuration.prototype.createConnection = function(skipDiscovery) {
	console.log("[NESTUITE-JS][CONFIGURATION][createConnection]");
	var _this = this;
	var createClient = denodeify(soap.createClient);
	console.log("[NESTUITE-JS][CONFIGURATION][createConnection] createClient: ", createClient);
	var connect = function(resolve, reject) {
		console.log("[NESTUITE-JS][CONFIGURATION][createConnection] connect ");
		_this._resolveWsdl();
		createClient(_this._wsdl, {
			attributesKey: '$attributes'
		})
			.then(function(client) {
				// console.log("[NESTUITE-JS][CONFIGURATION][createConnection][createClient] client 1: ", client);
				...
				_this.client = client;
				console.log("[NESTUITE-JS][CONFIGURATION][createConnection][createClient] client.wsdl.uri: ", client.wsdl.uri);
				console.log("[NESTUITE-JS][CONFIGURATION][createConnection][createClient] client.soapHeader: ", client.soapHeaders);
				resolve(client);
			})
			.catch(function(err) {
	...
	return new Promise(function(resolve, reject) {
		console.log("[NESTUITE-JS][CONFIGURATION][createConnection] if (skipDiscovery) ");
		// Just connect with current settings if don't need to do discovery
		if (skipDiscovery) {
			console.log("[NESTUITE-JS][CONFIGURATION][createConnection] if (skipDiscovery) IF ");
		...
		console.log("[NESTUITE-JS][CONFIGURATION][createConnection][authHeader] authHeader: ", authHeader);
		console.log("[NESTUITE-JS][CONFIGURATION][createConnection][authHeader] _this._restURL: ", _this._restURL);
		https.get({
			...
			console.log("[NESTUITE-JS][CONFIGURATION][createConnection][authHeader] res.req._header: ", res.req._header);
			var buffer = [];
